<table>
  ...
  <tr>
    <td>&emsp;<a href="./ARCHITECTURE.md">Architecture</a></td>
  </tr>
  <tr>
...
<h1>Blue Marble</h1>
<a href="" target="_blank" rel="noopener noreferrer"><img alt="Latest Version" src="https://img.shields.io/badge/Latest_Version-0.92.48-lightblue?style=flat"></a>
...
<p>
  ...
  <ul>
    ...
    <li>Showing a small green update arrow in the fork UI when the original Blue Marble repository has a newer version than the upstream version this fork is based on</li>
    <li>...and more!</li>
  </ul>
  ...
  Developers can also read <a href="./ARCHITECTURE.md">ARCHITECTURE.md</a> for the current source layer boundaries.
...
<p>
  ...
  <p>Color Filter keeps the most recently verified pixel counts in local userscript storage, so completed colors can remain hidden immediately after a Wplace reload when <b>Completed</b> is disabled. While the page is visible, Blue Marble rechecks map tiles currently on screen every 30 seconds even if Color Filter is closed; a modified pixel in a visible tile will therefore make that color appear again. Tiles outside the visible map are refreshed when you visit them.</p>
  <p>If you enable <b>Guard clicks</b> in the Color Filter while exactly one paintable template color is visible, Blue Marble blocks manual map clicks and Space-drag placement gestures that do not match that visible template color, while leaving normal map panning, low-zoom map points, and map marker clicks alone. If a Space-drag leaves the safe template path, Blue Marble cancels continuous placement with a synthetic Space release and requires a physical Space release before a new safe stroke can start. Automatically resuming the same held stroke is unsafe because Wplace may connect the two valid endpoints through invalid pixels. When a matching template pixel is clicked with the wrong Wplace color selected, it tries to select the visible template color before blocking the unsafe placement. It does not choose pixels or place outside the confirmed path automatically.</p>
  ...
  <p>If the eyedropper or <b>Guard clicks</b> does not behave as expected, open the browser console and enable focused debug logs with <code>localStorage.setItem('bm-debug-picker', 'true')</code> or <code>localStorage.setItem('bm-debug-guard', 'true')</code>, then refresh Wplace and try the click again. For live guard diagnostics without refreshing, run <code>window.postMessage({source: 'blue-marble-debug', endpoint: 'set-debug', channel: 'guard'}, '*')</code> and then <code>window.postMessage({source: 'blue-marble-debug', endpoint: 'status'}, '*')</code>. Disable local debug later with <code>localStorage.removeItem('bm-debug-picker')</code> and <code>localStorage.removeItem('bm-debug-guard')</code>.</p>
